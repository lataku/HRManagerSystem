...
<div>
    <form id="ff" method="post">
        <label for="did_select">编号：</label>
        <input class="easyui-numberbox" style="width: 150px; height: 25px;"
               name="did_select" id="did_select">
        <label for="dname_select">名称:</label>
        <input class="easyui-textbox" style="width: 150px; height: 25px;"
               name="dname_select" id="dname_select">
    </form>
...
<div id="dlg" class="easyui-dialog" title="修改部门信息"
        ...
        <table cellpadding="10">
            <tr>
                ...
                <td><input class="easyui-textbox" disabled="disabled"
                           type="text" name="did" id="did_edit"
                           data-options="required:true"></td>
            </tr>
            <tr>
                <td>名称：</td>
                <td><input class="easyui-textbox"
                           type="text" name="dname" id="dname_edit"
                           data-options="required:true"></td>
            </tr>
            <tr>
                <td>备注：</td>
                <td><input class="easyui-textbox" data-options="multiline:true"
                           name="dremark" id="dremark_edit"></td>
            </tr>
        </table>
    </form>
</div>
...
<script type="text/javascript">
    // 页面加载完成自动调用后
    $(function () {
        getData("${pageContext.request.contextPath}/selectAllDepts");
        getPagerbar();
    });

    // 模糊查询部门
    function submitForm1() {
        var did = $('#did_select').val();
        var dname = $('#dname_select').val();
        getData("${pageContext.request.contextPath}/selectDeptsLike", did, dname);
        getPagerbar();
    }

    // 获取部门数据
    function getData(url, did, dname) {
        $('#dg').datagrid({
            url: url,
            title: '部门列表',
            border: false,
        ...
        pager.pagination({
            buttons: [{
                ...
                handler: function () {
                    ...
                    if (null != row) {
                        $.messager.confirm('确定要删除', '确定要删除(' + row.dname + ')吗？', function (r) {
                            if (r) {
                                $.get('deleteDept?did=' + row.did, null, function (data) {
                                    // ajax异步返回结果
                ...
                handler: function () {
                    ...
                    if (null != row) {
                        ...
                        var row = $('#dg').datagrid('getSelected');
                        if (null != row) {
                            $('#did_edit').textbox('setValue', row.did);
                            $('#dname_edit').textbox('setValue', row.dname);
                            $('#dremark_edit').textbox('setValue', row.dremark);
                            $('#dlg').dialog('open');
                        }
                    } else {
                        $.messager.alert('警告', '请选中要操作的对象', 'warning');
    ...
    // 修改后保存数据的方法
    function saveEdit() {
        var did = $('#did_edit').val();
        var dname = $('#dname_edit').val();
        var dremark = $('#dremark_edit').val();
